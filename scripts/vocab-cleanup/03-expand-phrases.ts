...
async function loadPhase2(): Promise<Word[]> {
  const p = path.join(OUTPUT_DIR, 'vocabulary-02-deduped.ts')
...
const run = async () => {
  ...
  const outPath = path.join(OUTPUT_DIR, 'vocabulary-03-expanded.ts')
  const header = `// Auto-generated by scripts/vocab-cleanup/03-expand-phrases.ts\n// Phase 3: Multi-word phrases kept; content words added as standalone entries.\n\nimport type { Word } from '../src/data/vocabulary'\n\nexport const ALL_WORDS: Word[] = `
  fs.writeFileSync(outPath, header + JSON.stringify(expanded, null, 2) + '\n')
  ...
  console.log(`   Output: tmp/vocabulary-03-expanded.ts`)
