...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; }
    /* overflow-x:hidden is the safety net — the real fix is the cell
       wrapping below, which lets the wide tables shrink to the viewport
       instead of forcing the whole page to scroll sideways on mobile. */
    html, body { overflow-x: hidden; }
    body { font-family: system-ui, -apple-system, sans-serif; max-width: 800px; width: 100%; margin: 0 auto; padding: 2rem 1.5rem; color: #1c1917; line-height: 1.7; }
    ...
    code { background: #f3f4f6; padding: 0.1rem 0.35rem; border-radius: 0.25rem; font-size: 0.85em; overflow-wrap: anywhere; }
    ...
    .table { width: 100%; max-width: 100%; border-collapse: collapse; margin: 1rem 0; font-size: 0.95rem; }
    /* anywhere (not break-word) so long URLs/endpoints reduce the cell's
       min-content width — break-word alone leaves auto-layout tables wide
       enough to overflow on narrow screens. */
    .table th, .table td { border: 1px solid #e5e7eb; padding: 0.75rem; text-align: left; vertical-align: top; overflow-wrap: anywhere; word-break: break-word; }
...
<body>
  ...
  <table class="table">
    ...
    <tr><td>Quick Play guest resume hint (browser local storage only — no server-side state)</td><td>${RETENTION_PERIODS.quickPlayResumeHintMinutes} minutes after last activity</td><td>Automatic — silently dropped by the browser when the timer expires</td></tr>
