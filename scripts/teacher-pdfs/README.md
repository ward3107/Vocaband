...
Each content file exports a doc object with `en`, `he`, `ar`, `ru` siblings.
The shape is described in `render.mjs`.

> **Note:** the old multi-page `teacher-guide` and `quick-start` PDFs were
> retired in favour of the visual single-page handouts generated by
> `scripts/presentation-pdf/build.mjs`
> (`Vocaband-Teacher-OnePager-HE.pdf` / `-AR.pdf`).

## Generate

```bash
# All PDFs (3 docs × 4 languages)
...
node scripts/teacher-pdfs/build.mjs --doc=student-guide
