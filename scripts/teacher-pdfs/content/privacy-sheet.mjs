...
const ru = {
  cover: {
    kind: 'Памятка по конфиденциальности и данным',
    code: 'PR · ЧЕРНОВИК',
    title: 'Конфиденциальность и данные',
    strap: 'Краткое и понятное резюме для администрации школы.',
    sub: 'Что собирает Vocaband, зачем, где это хранится и сколько. На следующей странице — однополосная версия для учительской.',
    pills: ['Черновик v1', 'Ожидается юридическая проверка', 'Хостинг в ЕС', 'Без рекламы для детей'],
    footerNote: 'Черновик в ожидании юриста — не распространять',
  },
  pages: [
    {
      heading: 'Что мы собираем',
      emoji: '🛡️',
      intro: 'Vocaband спроектирован так, чтобы собирать минимум, необходимый для работы класса с лексикой. Список ниже — исчерпывающий.',
      blocks: [
        { type: 'draftBanner', tag: 'ЧЕРНОВИК', text: 'Этот документ — рабочий черновик в ожидании юридической проверки. Финальная подписанная версия — это та, что должна попадать в школы, Министерство образования или к родителям.' },
        { type: 'h3', text: 'Учителя' },
        { type: 'ul', items: [
          'Email — только для входа (Google, Microsoft или одноразовый код по почте).',
          'Отображаемое имя — берётся у поставщика OAuth, можно редактировать.',
          'Названия классов, настройки заданий и списки слов, которые создаёт учитель.',
        ]},
        { type: 'h3', text: 'Ученики' },
        { type: 'ul', items: [
          'Отображаемое имя — задаётся учителем при добавлении. **Email или телефон ученика не запрашиваются.**',
          'Прогресс по каждому слову (верно / неверно / число попыток), XP, счётчик серии, выбор аватара.',
          'Метки времени игровых сессий (чтобы учитель видел, кто играл сегодня).',
        ]},
        { type: 'h3', text: 'Что мы НЕ собираем' },
        { type: 'ul', items: [
          'Никаких рекламных идентификаторов и трекинговых пикселей третьих сторон.',
          'Никаких контактов родителей, домашних адресов, телефонов или фотографий.',
          'Никаких данных микрофона или камеры — за исключением случая, когда учитель добровольно использует камеру для OCR печатного списка слов: снимок обрабатывается и удаляется.',
        ]},
        { type: 'callout', tag: 'info', label: 'Простыми словами', body: 'Ребёнок может пользоваться Vocaband, никогда не сообщая приложению свой email, телефон или какие-либо личные контакты. Учитель — единственная учётная запись, идентифицируемая как взрослый.' },
      ],
    },
    {
      heading: 'Где хранится и как долго',
      emoji: '🌍',
      intro: 'Где находятся данные, кто к ним имеет доступ и что происходит в конце учебного года.',
      blocks: [
        { type: 'draftBanner', tag: 'ЧЕРНОВИК', text: 'Ожидается юридическая проверка.' },
        { type: 'h3', text: 'Место хранения' },
        { type: 'p', text: 'Все записи учителей и учеников хранятся в **Европейском союзе** (Франкфурт, Германия) на управляемой Postgres от Supabase. Аудиофайлы и изображения — в Cloudflare R2 с резидентностью в ЕС.' },
        { type: 'h3', text: 'Кто имеет доступ к данным' },
        { type: 'table', headers: ['Кто', 'Что видит'], rows: [
          ['Учитель, создавший класс', 'Весь прогресс, XP и ответы своих учеников.'],
          ['Соучителя, добавленные основным учителем', 'То же, что и основной учитель.'],
          ['Инженерная команда Vocaband', 'Агрегированные метрики для улучшения продукта. Доступ к данным конкретного ученика — только через тикет поддержки и логируется.'],
          ['Ученики', 'Только свой собственный прогресс. Ответы и имена других учеников вне их класса не видны.'],
        ]},
        { type: 'h3', text: 'Сколько мы храним' },
        { type: 'ul', items: [
          'Активные классы — пока активен аккаунт учителя.',
          'Закрытые классы — до **24 месяцев** с последней активности, затем анонимизируются.',
          'Учитель может удалить свой аккаунт в любое время; это удаляет все классы и записи учеников, связанные с ним.',
        ]},
        { type: 'h3', text: 'Безопасность' },
        { type: 'ul', items: [
          'Весь трафик шифруется (TLS 1.3, **SSL Labs A+**).',
          'Постраничная авторизация в БД (RLS) — учитель технически может прочитать только строки своих классов.',
          'Ежегодный независимый пентест (запланирован на **лето 2026**).',
        ]},
        { type: 'callout', tag: 'tip', label: 'Контакт', body: 'Вопросы по конфиденциальности: **privacy@vocaband.com**. Запросы на доступ к данным или их удаление обрабатываются в течение 7 рабочих дней.' },
      ],
    },
  ],
};

export const privacySheet = { key: 'privacy-sheet', emoji: '🛡️', en, he, ar, ru };
