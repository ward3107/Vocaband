...
const ru = {
  cover: {
    ...
    code: 'QS · v1',
    title: 'Быстрый старт',
    strap: 'От нуля до первого урока за 10 минут.',
    sub: 'Одностраничная памятка для новых учителей. Передайте коллеге или повесьте на стену.',
    pills: ['10 минут', 'Без установки', 'Работает на любом устройстве'],
    footerNote: 'vocaband.com/for-teachers',
  },
  pages: [
    {
      heading: '5 шагов',
      emoji: '⚡',
      intro: 'Сделайте это один раз. На подготовку следующих уроков уйдёт 30 секунд.',
      blocks: [
        { type: 'steps', items: [
          { title: 'Войдите', body: 'Откройте **vocaband.com**, нажмите **Вход для учителей** и войдите через Google (или Microsoft).' },
          { title: 'Создайте класс', body: 'Выберите название и уровень. Выберите **Set 1, 2, 3** или **свой список**.' },
          { title: 'Добавьте слова', body: 'Отметьте слова из списка, вставьте свои или сфотографируйте печатный список (OCR).' },
          { title: 'Поделитесь кодом', body: 'Покажите **6-значный код класса** или **QR-код** на проекторе. Ученики заходят через vocaband.com.' },
          { title: 'Играйте вместе', body: 'Запустите **Live Challenge** для всего класса или дайте ученикам заниматься в своём темпе из их панели.' },
        ]},
        { type: 'callout', tag: 'tip', label: 'Совет', body: 'На первом уроке стремитесь к **10 словам и 2 режимам**. Меньше — лучше. Усложняйте на второй неделе.' },
        { type: 'h3', text: 'Что видят ученики' },
        { type: 'ul', items: [
          'Свой **аватар**, **XP** и **🔥 серию** — за ежедневную игру, не за скорость.',
          '**Ежедневный сундук** — награда просто за то, что зашёл.',
          '**Прогресс по каждому слову** — чтобы знать, что повторить.',
        ]},
        { type: 'callout', tag: 'info', label: 'Нужна полная картина?', body: 'Прочитайте PDF **Руководство для учителя** — тот же сайт, то же место, где вы взяли эту памятку.' },
      ],
    },
  ],
};

export const quickStart = { key: 'quick-start', emoji: '⚡', en, he, ar, ru };
