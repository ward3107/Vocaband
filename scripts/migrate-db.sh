...
#   - A .env.migrate file in repo root (see .env.migrate.example).
#
# Connection strings are built at runtime from separate host/password
# fields; we never store a full DB URL in a committed file because
# GitHub's secret scanner flags postgres:// URLs even when the password
# is a placeholder.
#
# Usage:
...
if [ ! -f .env.migrate ]; then
  ...
  echo "  and fill in the TOKYO_* and FRANKFURT_* fields."
  exit 1
fi
...
: "${TOKYO_DB_HOST:?TOKYO_DB_HOST is empty in .env.migrate}"
: "${TOKYO_DB_PASSWORD:?TOKYO_DB_PASSWORD is empty in .env.migrate}"
: "${FRANKFURT_DB_HOST:?FRANKFURT_DB_HOST is empty in .env.migrate}"
: "${FRANKFURT_DB_PASSWORD:?FRANKFURT_DB_PASSWORD is empty in .env.migrate}"

# Assemble the two URIs from their parts. We URL-encode the password
# because Supabase-generated passwords sometimes contain characters like
# `+` or `/` that would otherwise break the URI parser.
urlencode() {
  local s=$1 out="" c
  for (( i = 0; i < ${#s}; i++ )); do
    c=${s:i:1}
...
TOKYO_DB_URL="postgresql://postgres:$(urlencode "$TOKYO_DB_PASSWORD")@${TOKYO_DB_HOST}:5432/postgres"
FRANKFURT_DB_URL="postgresql://postgres:$(urlencode "$FRANKFURT_DB_PASSWORD")@${FRANKFURT_DB_HOST}:5432/postgres"
