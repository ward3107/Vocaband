...
| Latin web fonts self-hosted | Done (2026-05-19) | Plus Jakarta Sans (variable) + Be Vietnam Pro (weights 400/500/700/900), latin subset, shipped as 5 woff2 files in `public/fonts/` and declared via `@font-face` in `src/index.css`.  Removed the Latin Google Fonts `<link>` from `index.html` and the matching media-swap helper in `boot-debug.js`.  `THIRD_PARTY_REGISTRY` Google Fonts entry narrowed to "Heebo + Fredoka — Hebrew/Arabic only".  CSP rules unchanged: RTL fonts still lazy-load from Google Fonts when the visitor's language is HE/AR.  Self-hosting the RTL fonts is the next step (separate PR, needs subset selection for Hebrew range). |
