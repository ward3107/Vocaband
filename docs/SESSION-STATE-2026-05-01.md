...
| Area | Done | Pending |
|---|---|---|
| **Resend SMTP** | Account, domain DKIM/SPF verified, custom SMTP enabled in Supabase | Email-template tweak (Step 6 in `docs/RESEND-SMTP-SETUP.md`) |
| **Game-mode redesigns** | Phase 3a–3i shipped + all merged | None |
| **New game mode** | Memory Flip merged | Optional: enable it as a default in `allowedModes` lists |
| **QP teacher monitor** | QR icon, footer removed, podium owns the screen | None |
| **QP student flow** | Language picker step shipped + merged | More avatars branch NOT yet merged |
| **QP resume bug** | Fixed (score restored from localStorage hint) | NOT yet merged to main |
| **Saved word groups across auth methods** | Hook + migration written | NOT yet merged to main; **migration NOT yet applied to DB** |
| **Errors after each game mode** | Investigation pending | **Need the actual error message from you** |
...
### ⏳ Pushed but NOT merged yet

| Branch | What | Why review carefully before merge |
|---|---|---|
| `claude/qp-more-avatars` | Tabbed avatar picker (~30 emojis × 6 groups + Geometric tab with 30 lucide icons) | Touches `QuickPlayMonitor.tsx` rendering — verify rank-4+ tile renders correctly with QPAvatar component |
| `claude/qp-resume-continue-fix` | Restores QP score on rescan so kid keeps playing forward | App.tsx state initializer change — verify fresh joins still start at 0 |
| `claude/saved-groups-by-email` | Re-key saved word groups by email so magic-link + Google OAuth share groups | **Requires SQL migration paste** — see below |
...
## What I'd ship next, given a free hand

In priority order:

1. **Capture + fix the #5 regression** — blocking real users.  Need the error text first.
2. **Merge the 3 pending branches** in this order: avatars → resume-fix → saved-groups (saved-groups last so the migration can run right after).
3. **Apply the saved-groups SQL migration** — one paste, ~5 sec, restores existing teachers' visible group lists.
4. **Apply Resend's email-template tweak** — 5 min in Supabase dashboard.
5. **Microsoft Azure sign-in** — operator config + UI button.  Highest-impact provider expansion since it covers Israeli school accounts (`*.edu.gov.il`, `*.tlv.org.il`) that are mostly Microsoft-tenant'd.

Apple sign-in, identity linking, and the deeper modes (additional games beyond Memory Flip) are nice-to-haves that can wait.
