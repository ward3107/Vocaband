  ...
  return (
    <section className="space-y-2.5">
      <header className={`flex items-baseline gap-2 px-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
        ...
        <h2 className="text-lg font-black tracking-tight text-[color:var(--ios-label)]">{title}</h2>
        {subtitle && (
          <span className="text-xs font-medium text-[color:var(--ios-label-secondary)]">{subtitle}</span>
        )}
      </header>
      <div
