...
export const accessibilityTranslations: Record<Language, {
  ...
  en: {
    ...
    standards: { title: "Standards Compliance", items: ["WCAG 2.0 Level AA — verified (Israeli Standard IS 5568 baseline; full completion summary at docs/WCAG_COMPLETION_SUMMARY.md)", "WCAG 2.1 Level AA — in progress, per-criterion readiness tracked in the same completion summary (EU Accessibility Act 2025 alignment, EN 301 549)", "Section 508 — US federal accessibility requirements"] },
  ...
  he: {
    ...
    standards: { title: "עמידה בתקנים", items: ["WCAG 2.0 רמה AA — אומת (תקן ישראלי ת\"י 5568 כבסיס; סיכום מלא ב-docs/WCAG_COMPLETION_SUMMARY.md)", "WCAG 2.1 רמה AA — בתהליך, מצב מוכנות לפי קריטריון נעקב באותו סיכום (התאמה לחוק הנגישות האירופי 2025, EN 301 549)", "Section 508 — דרישות נגישות פדרליות של ארה\"ב"] },
  ...
  ar: {
    ...
    standards: { title: "الامتثال للمعايير", items: ["WCAG 2.0 المستوى AA — مُتحقَّق منه (المعيار الإسرائيلي IS 5568 كأساس؛ ملخَّص الإنجاز الكامل في docs/WCAG_COMPLETION_SUMMARY.md)", "WCAG 2.1 المستوى AA — قيد التنفيذ، يتم تتبُّع جاهزية كل معيار في الملخَّص نفسه (مواءمة قانون إمكانية الوصول للاتحاد الأوروبي 2025، EN 301 549)", "القسم 508 — متطلبات إمكانية الوصول الفيدرالية الأمريكية"] },
  ...
  ru: {
    title: "Заявление о доступности", lastUpdated: "Последнее обновление: апрель 2026 г.", back: "Назад",
    commitment: { title: "Наше обязательство", text: "Vocaband стремится обеспечить цифровую доступность для всех пользователей, включая учащихся, учителей и родителей с ограниченными возможностями. Мы работаем над соответствием WCAG 2.0 уровня AA и израильскому стандарту IS 5568 по веб-доступности. Наша цель — предоставить инклюзивный учебный опыт, в котором каждый учащийся может эффективно осваивать английскую лексику." },
    features: { title: "Функции доступности",
      toolbar: { title: "Панель доступности", intro: "Плавающая кнопка доступности (правый нижний угол) предоставляет 10 настраиваемых параметров:", items: ["Размер шрифта — масштабирование текста от 88% до 200%", "Высокий контраст — чёрный фон с золотым текстом и голубыми ссылками", "Шрифт для дислексии — переключение на OpenDyslexic или Comic Sans MS", "Читаемый шрифт — переключение на Open Sans / Helvetica Neue", "Уменьшение движения — минимизация всех анимаций и переходов", "Выделение ссылок — подчёркивание и рамки для всех ссылок", "Большой курсор — настраиваемый курсор 32px с минимальными целевыми областями касания 44px", "Высота строки — настройка межстрочного интервала (1.5–2.0)", "Межбуквенный интервал — настройка интервала между буквами (от обычного до очень широкого)", "Индикаторы фокуса — улучшенные контуры фокуса на всех интерактивных элементах"] },
      keyboard: { title: "Навигация с клавиатуры", items: ["Ссылки пропуска в верхней части каждой страницы", "Все интерактивные элементы доступны клавишей Tab", "Кнопки реагируют на клавиши Enter и Space", "Модальные окна захватывают фокус и закрываются клавишей Escape", "Видимые индикаторы фокуса с синей рамкой"] },
      screenReader: { title: "Программы экранного доступа", items: ["Ориентиры ARIA для структуры страницы", "Описательные метки на кнопках, значках и элементах управления", "Живые области для обновлений динамического контента", "Поля формы со связанными метками и объявлениями об ошибках"] },
      rtl: { title: "Поддержка языков и RTL", items: ["Полная поддержка английского, иврита и арабского", "Автоматическая раскладка справа налево для иврита и арабского", "Обработка двунаправленного текста в отображении лексики и играх"] },
      visual: { title: "Визуальный дизайн", items: ["Коэффициенты цветового контраста соответствуют требованиям WCAG AA", "Информация не передаётся только цветом", "Учитывает системную настройку prefers-reduced-motion", "Текст можно увеличить до 200% без потери содержания", "Адаптивный дизайн от 320px до настольного"] },
      audio: { title: "Звук и произношение", items: ["Произношение слов запускается действием пользователя, а не автоматически", "У всего аудиоконтента есть визуальные эквиваленты", "Регуляторы громкости доступны в сессиях Quick Play"] },
    },
    standards: { title: "Соответствие стандартам", items: ["WCAG 2.0 уровень AA — проверено (израильский стандарт IS 5568 как базовый; полный отчёт о завершении в docs/WCAG_COMPLETION_SUMMARY.md)", "WCAG 2.1 уровень AA — в процессе, готовность по каждому критерию отслеживается в том же отчёте (соответствие Закону ЕС о доступности 2025 г., EN 301 549)", "Section 508 — федеральные требования США по доступности"] },
    limitations: { title: "Известные ограничения", items: ["Некоторые игровые режимы требуют быстрых реакций, что может быть сложно для пользователей, которым нужно дополнительное время. Режим карточек не имеет ограничения по времени.", "Присоединение к Quick Play через QR-код требует камеры. Учащиеся также могут ввести код сессии вручную.", "Вход через Google OAuth и шрифты Google загружаются из внешних служб со своими политиками доступности."] },
    testing: { title: "Тестирование", intro: "Мы тестируем доступность с помощью автоматизированных инструментов и ручного тестирования:", items: ["Аудит доступности Chrome Lighthouse", "Тестирование навигации только с клавиатуры", "Тестирование с программами экранного доступа NVDA и VoiceOver", "Тестирование режимов высокого контраста и уменьшения движения", "Тестирование мобильных устройств на iOS и Android"] },
    contact: { title: "Обратная связь и контакты", intro: "Если вы столкнулись с препятствиями для доступа, пожалуйста, свяжитесь с нами:", email: "accessibility@vocaband.com", response: "Мы стремимся ответить в течение 5 рабочих дней и решить проблемы в течение 30 дней." },
  },
};
