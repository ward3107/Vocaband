...
export const modeIntroT: Record<Language, ModeIntroStrings> = {
  en: {
    modes: withIcons({
      ...
      "category-race": { title: "Category Race", steps: ["A letter is rolled — type one answer per category", "Hebrew or Arabic still counts, but earns fewer points", "Submit early to bank time bonus before the clock hits zero"] },
      dreidel: { title: "Dreidel Blitz", steps: ["The dreidel spins to a letter", "Type any English word that starts with it", "First to answer wins the round!"] },
  ...
  he: {
    modes: withIcons({
      ...
      "category-race": { title: "מרוץ קטגוריות", steps: ["מגרילים אות — מלא תשובה בכל קטגוריה", "תשובה בעברית או ערבית נחשבת אבל מזכה בפחות נקודות", "סיים מוקדם כדי לקבל בונוס זמן"] },
      dreidel: { title: "סביבון בזק", steps: ["הסביבון נופל על אות", "הקלד מילה באנגלית שמתחילה באות הזו", "הראשון שעונה זוכה בסיבוב!"] },
  ...
  ar: {
    modes: withIcons({
      ...
      "category-race": { title: "سباق الفئات", steps: ["يُختار حرف — اكتب إجابة لكل فئة", "العبرية أو العربية تُحتسب لكن تكسب نقاطًا أقل", "أنهِ مبكرًا للحصول على مكافأة وقت"] },
      dreidel: { title: "دريدل البرق", steps: ["تدور الدريدل لتقف على حرف", "اكتب أي كلمة إنجليزية تبدأ بهذا الحرف", "الأول الذي يجيب يفوز بالجولة!"] },
