...
import { StudentLoginView } from "./features/student/StudentLoginView";
...
import { GameFinishedView } from "./features/game/GameFinishedView";
import { GameModeIntroView } from "./features/game/GameModeIntroView";
import { GameModeSelectionView } from "./features/game/GameModeSelectionView";
import { GamePlayView } from "./features/game/GamePlayView";
...
import { StudentDashboardView } from "./features/student/StudentDashboardView";
import { QuickPlayMonitorView } from "./features/quick-play/QuickPlayMonitorView";
import { QuickPlaySetupView } from "./features/quick-play/QuickPlaySetupView";
import { QuickPlayStudentView } from "./features/quick-play/QuickPlayStudentView";
...
function AppContent({ onSessionRestoredRef, onSignedOutRef, onNoSessionRef, onAuthErrorRef }: AppContentProps) {
  ...
  if (view === "student-account-login") {
    return (
      <StudentLoginView
        ...
        avatarCategories={AVATAR_CATEGORIES}
  ...
  if (user && view === "privacy-settings") {
    return <PrivacySettingsView consentModal={consentModal} setView={setView} />;
  }

  // --- SHOP VIEW ---
  if (user?.role === "student" && view === "shop") {
    return <ShopView xp={xp} setXp={setXp} shopTab={shopTab} setShopTab={setShopTab} setView={setView} />;
  }
  ...
  if (user?.role === "teacher" && view === "teacher-dashboard") {
    return (
      <TeacherDashboardView
        ...
        endQuickPlayModal={endQuickPlayModal}
        setEndQuickPlayModal={setEndQuickPlayModal}
        quickPlayActiveSession={quickPlayActiveSession}
  ...
  if (view === "quick-play-setup") {
    return (
      <QuickPlaySetupView
        ...
        quickPlaySearchQuery={quickPlaySearchQuery}
        setQuickPlaySearchQuery={setQuickPlaySearchQuery}
        ...
        quickPlayWordEditorOpen={quickPlayWordEditorOpen}
        setQuickPlayWordEditorOpen={setQuickPlayWordEditorOpen}
        quickPlaySessionCode={quickPlaySessionCode}
        setQuickPlaySessionCode={setQuickPlaySessionCode}
        quickPlayActiveSession={quickPlayActiveSession}
        setQuickPlayActiveSession={setQuickPlayActiveSession}
        showQuickPlayPreview={showQuickPlayPreview}
        setShowQuickPlayPreview={setShowQuickPlayPreview}
        quickPlayPreviewAnalysis={quickPlayPreviewAnalysis}
        setQuickPlayPreviewAnalysis={setQuickPlayPreviewAnalysis}
        ...
        handleQuickPlayPreviewConfirm={handleQuickPlayPreviewConfirm}
        handleQuickPlayPreviewCancel={handleQuickPlayPreviewCancel}
        setView={setView}
      />
    );
  }

  // Removed ~810 lines of inline quick-play-setup JSX (now in QuickPlaySetupView)


  if (view === "quick-play-teacher-monitor") {
    return (
      <QuickPlayMonitorView
        quickPlayActiveSession={quickPlayActiveSession}
  ...
  if (isFinished) {
    return (
      <GameFinishedView
        ...
        saveError={saveError}
        onExitGame={handleExitGame}
        onBackToDashboard={() => { setIsFinished(false); setScore(0); setCurrentIndex(0); setMistakes([]); setFeedback(null); setShowModeSelection(true); setView("student-dashboard"); }}
      />
    );
  }
  ...
  if (showModeIntro) {
    return (
      <GameModeIntroView
        gameMode={gameMode}
        introLang={introLang}
        setIntroLang={setIntroLang}
        onStart={() => setShowModeIntro(false)}
        onBack={() => { setShowModeIntro(false); setShowModeSelection(true); }}
      />
    );
  }
  ...
  const handleClearSentence = (words: string[]) => {
    setBuiltSentence([]);
    setAvailableWords(words);
  };
  ...
  return (
    <GamePlayView
      ...
      onToggleLanguage={() => setTargetLanguage(targetLanguage === "hebrew" ? "arabic" : "hebrew")}
      gameMode={gameMode}
      ...
      onUseRevealLetter={handleUseRevealLetter}
