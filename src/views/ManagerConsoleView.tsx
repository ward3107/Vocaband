...
const tip = { borderRadius: 12, border: '1px solid #e7e5e4', fontSize: 12, boxShadow: '0 4px 16px rgba(0,0,0,0.08)' };
...
function ChartCard({ title, children, ta }: { title: string; children: ReactNode; ta: string }) {
  return (
    <div className="rounded-2xl bg-white shadow-sm p-4">
      <div className={`text-sm font-semibold text-stone-700 mb-3 ${ta}`}>{title}</div>
...
function Spinner({ msg }: { msg: string }) {
  return <div className="flex flex-col items-center justify-center py-20 text-stone-400"><RefreshCw className="w-7 h-7 animate-spin mb-3" /><span className="text-sm font-medium">{msg}</span></div>;
}

interface Props { user: AppUser | null; setView: Dispatch<SetStateAction<View>>; setUser: Dispatch<SetStateAction<AppUser | null>>; }

export default function ManagerConsoleView({ setView, setUser }: Props) {
  ...
  return (
    <div dir={dir} className={`min-h-screen bg-stone-50 flex ${rowR}`}>
      {/* Sidebar */}
      <aside className="w-64 shrink-0 bg-white flex flex-col" style={{ [isRTL ? 'borderLeft' : 'borderRight']: '1px solid #e7e5e4' } as CSSProperties}>
        <div className={`p-5 flex items-center gap-3 ${rowR}`}>
          ...
          <div className={`min-w-0 ${ta}`}><div className="font-bold text-stone-800 truncate text-sm">{overview?.school?.name ?? 'Vocaband'}</div><div className="text-xs text-stone-400">{t.role}</div></div>
        </div>
          ...
          {nav.map((n) => {
            ...
            return (
              <button key={n.id} type="button" onClick={() => go(n.id)} style={{ touchAction: 'manipulation' }}
                className={`w-full flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition-colors ${rowR} ${ta} ${active ? 'text-white bg-gradient-to-r from-indigo-500 to-violet-500 shadow-sm shadow-violet-500/30' : 'text-stone-600 hover:bg-stone-100'}`}>
                <n.icon className="w-4 h-4 shrink-0" /><span className="flex-1">{n.label}</span>
              </button>
            );
          })}
        </nav>
        <div className="p-3"><button type="button" onClick={() => void signOut()} className={`w-full flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-stone-500 hover:bg-stone-100 ${rowR} ${ta}`}><LogOut className="w-4 h-4" /><span className="flex-1">{t.signOut}</span></button></div>
      </aside>

      {/* Main */}
      <main className="flex-1 min-w-0 p-6">
        <div className={`flex items-center justify-between mb-5 ${rowR}`}>
          <h1 className={`text-xl font-extrabold text-stone-800 ${ta}`}>
          ...
          <div className={`flex items-center gap-2 ${rowR}`}>
            <span className="text-xs font-medium text-stone-500 bg-white rounded-lg px-3 py-1.5 shadow-sm">{t.range}</span>
            <button type="button" onClick={() => void loadOverview()} className="inline-flex items-center gap-1.5 text-xs font-semibold text-stone-600 bg-white rounded-lg px-3 py-1.5 shadow-sm hover:bg-stone-50"><RefreshCw className={`w-3.5 h-3.5 ${loadingOverview ? 'animate-spin' : ''}`} />{t.refresh}</button>
          </div>
        </div>

        {loadingOverview ? <Spinner msg={t.loading} />
          : !overview?.school || !totals ? (
            <div className={`rounded-3xl bg-white shadow-sm p-8 text-center ${ta}`}>
              <div className="flex items-center justify-center w-14 h-14 rounded-2xl bg-amber-100 text-amber-600 mx-auto mb-4"><AlertTriangle className="w-7 h-7" /></div>
            ...
            <>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <ChartCard title={t.chEngagement} ta={ta}>
                      <ResponsiveContainer><AreaChart data={overview.engagement14} margin={{ top: 6, right: 10, bottom: 0, left: -18 }}>
                        ...
                        <CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" vertical={false} /><XAxis dataKey="d" tick={{ fontSize: 9 }} stroke="#78716c" interval={1} /><YAxis tick={{ fontSize: 10 }} stroke="#78716c" allowDecimals={false} /><Tooltip contentStyle={tip} />
                    ...
                    <ChartCard title={t.chGames} ta={ta}>
                      <ResponsiveContainer><BarChart data={overview.engagement14} margin={{ top: 6, right: 10, bottom: 0, left: -18 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" vertical={false} /><XAxis dataKey="d" tick={{ fontSize: 9 }} stroke="#78716c" interval={1} /><YAxis tick={{ fontSize: 10 }} stroke="#78716c" allowDecimals={false} /><Tooltip contentStyle={tip} cursor={{ fill: '#f5f5f4' }} />
                    ...
                    <ChartCard title={t.chXp} ta={ta}>
                      <ResponsiveContainer><BarChart data={overview.xp_by_teacher} layout="vertical" margin={{ top: 0, right: 12, bottom: 0, left: 8 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" horizontal={false} /><XAxis type="number" tick={{ fontSize: 10 }} stroke="#78716c" /><YAxis type="category" dataKey="name" tick={{ fontSize: 11 }} stroke="#78716c" width={64} /><Tooltip contentStyle={tip} cursor={{ fill: '#f5f5f4' }} />
                        <Bar dataKey="xp" fill="#f59e0b" radius={[0, 4, 4, 0]} />
                      </BarChart></ResponsiveContainer>
                    </ChartCard>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className={`rounded-2xl bg-white shadow-sm p-4 ${ta}`}><div className={`flex items-center gap-2 text-emerald-600 mb-2 ${rowR}`}><TrendingUp className="w-4 h-4" /><span className="text-xs font-bold uppercase tracking-wide">{t.hMostActive}</span></div><div className="font-bold text-stone-800">{mostActive?.display_name ?? t.noData}</div><div className="text-sm text-stone-500">{mostActive ? `${mostActive.active_students_7d} ${t.colActive}` : ''}</div></div>
                    <div className={`rounded-2xl bg-white shadow-sm p-4 ${ta}`}><div className={`flex items-center gap-2 text-amber-600 mb-2 ${rowR}`}><AlertTriangle className="w-4 h-4" /><span className="text-xs font-bold uppercase tracking-wide">{t.hNeedsAttention}</span></div><div className="font-bold text-stone-800">{dormantClasses.length > 0 ? dormantClasses.slice(0, 3).map((c) => c.name).join(', ') : t.noData}</div><div className="text-sm text-stone-500">{dormantClasses.length > 0 ? t.dormant : ''}</div></div>
                    <div className={`rounded-2xl bg-white shadow-sm p-4 ${ta}`}><div className={`flex items-center gap-2 text-fuchsia-600 mb-2 ${rowR}`}><Award className="w-4 h-4" /><span className="text-xs font-bold uppercase tracking-wide">{t.hTopStudent}</span></div><div className="font-bold text-stone-800">{topTeacher?.name ?? t.noData}</div><div className="text-sm text-stone-500">{topTeacher ? `${topTeacher.xp.toLocaleString()} XP` : ''}</div></div>
                  </div>
                </div>
              )}
              ...
              {tab === 'teachers' && !teacherId && (
                <div className="space-y-3">
                  {overview.teachers.map((tc) => (
                    <motion.button key={tc.uid} type="button" whileHover={{ scale: 1.01 }} onClick={() => openTeacher(tc.uid)} className={`w-full rounded-2xl bg-white shadow-sm p-4 flex items-center gap-3 ${rowR}`}>
                      <div className="flex items-center justify-center w-11 h-11 rounded-2xl bg-gradient-to-br from-indigo-100 to-fuchsia-100 text-indigo-600 font-bold shrink-0">{(tc.display_name || '?')[0]}</div>
                      <div className={`flex-1 min-w-0 ${ta}`}><div className="font-semibold text-stone-800">{tc.display_name}</div><div className="text-xs text-stone-400">{tc.class_count} {t.colClasses} · {tc.student_count} {t.colStudents} · {tc.active_students_7d} {t.colActive}</div></div>
                      {daysSince(tc.last_activity) > 14 && <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 text-amber-700 text-xs font-semibold px-2.5 py-1"><AlertTriangle className="w-3 h-3" />{t.dormant}</span>}
                      <Caret className="w-4 h-4 text-stone-300 shrink-0" />
                    </motion.button>
                  ))}
                </div>
              )}
              ...
              {tab === 'teachers' && teacherId && (
                !teacherDetail ? <Spinner msg={t.loading} /> : (
                  <div className="space-y-5">
                    <button type="button" onClick={() => setTeacherId(null)} className={`inline-flex items-center gap-1.5 text-sm font-semibold text-indigo-600 ${rowR}`}>{isRTL ? <ArrowRight className="w-4 h-4" /> : <ArrowLeft className="w-4 h-4" />}{t.backTeachers}</button>
                    <div className={`rounded-2xl bg-white shadow-sm p-5 flex items-center gap-4 ${rowR}`}>
                      ...
                      <div className={ta}><div className="text-lg font-extrabold text-stone-800">{teacherDetail.teacher.display_name}</div>{teacherDetail.teacher.email && <div className={`flex items-center gap-1 text-sm text-stone-400 ${rowR}`}><Mail className="w-3.5 h-3.5" />{teacherDetail.teacher.email}</div>}</div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      ...
                      <ChartCard title={t.tdPerClass} ta={ta}>
                        <ResponsiveContainer><BarChart data={teacherDetail.per_class} margin={{ top: 6, right: 10, bottom: 0, left: -18 }}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" vertical={false} /><XAxis dataKey="name" tick={{ fontSize: 11 }} stroke="#78716c" /><YAxis tick={{ fontSize: 10 }} stroke="#78716c" allowDecimals={false} /><Tooltip contentStyle={tip} cursor={{ fill: '#f5f5f4' }} />
                          <Bar dataKey="students" fill="#10b981" radius={[4, 4, 0, 0]} />
                        </BarChart></ResponsiveContainer>
                      </ChartCard>
                    </div>
                    {teacherDetail.top_students.length > 0 && (
                      <div className="rounded-2xl bg-white shadow-sm p-4"><div className={`text-sm font-semibold text-stone-700 mb-3 ${ta}`}>{t.tdTopStudents}</div><div className="space-y-2">{teacherDetail.top_students.map((s, i) => (<div key={`${s.name}-${i}`} className={`flex items-center gap-3 ${rowR}`}><div className="w-6 h-6 rounded-lg bg-stone-100 text-stone-500 text-xs font-bold flex items-center justify-center">{i + 1}</div><div className={`flex-1 text-sm font-medium text-stone-700 ${ta}`}>{s.name}</div><div className="text-sm font-semibold text-amber-600">{s.xp.toLocaleString()} XP</div></div>))}</div></div>
                    )}
                  </div>
                )
              )}
              ...
              {tab === 'classes' && !classId && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {overview.classes.map((c) => (
                    <motion.button key={c.id} type="button" whileHover={{ scale: 1.02 }} onClick={() => openClass(c.id)} className={`rounded-2xl bg-white shadow-sm p-4 ${ta}`}>
                      <div className={`flex items-center justify-between mb-3 ${rowR}`}>
                        ...
                        {daysSince(c.last_activity) > 14 ? <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 text-amber-700 text-xs font-semibold px-2 py-1"><AlertTriangle className="w-3 h-3" />{t.dormant}</span>
                          : <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 text-emerald-700 text-xs font-semibold px-2 py-1"><CheckCircle2 className="w-3 h-3" />{c.completion}%</span>}
                      </div>
                      {c.teacher_name && <div className="text-xs text-stone-400 mb-2 truncate">{c.teacher_name}</div>}
                      <div className={`flex flex-wrap gap-x-4 gap-y-1 text-sm ${rowR}`}>
                        <span className="text-stone-600"><b className="text-stone-800">{c.students}</b> {t.colStudents}</span>
                        {c.avg_score != null && <span className="text-stone-600"><b className="text-stone-800">{c.avg_score}</b> {t.kAvg}</span>}
                        <span className="text-stone-600"><b className="text-stone-800">{c.active_7d}</b> {t.colActive}</span>
                      </div>
                      <div className="mt-3 h-1.5 rounded-full bg-stone-100 overflow-hidden"><div className="h-full rounded-full bg-gradient-to-r from-emerald-400 to-teal-500" style={{ width: `${c.completion}%` }} /></div>
                    </motion.button>
                  ))}
                </div>
              )}
              ...
              {tab === 'classes' && classId && (
                !classDetail ? <Spinner msg={t.loading} /> : (
                  <div className="space-y-5">
                    <button type="button" onClick={() => setClassId(null)} className={`inline-flex items-center gap-1.5 text-sm font-semibold text-indigo-600 ${rowR}`}>{isRTL ? <ArrowRight className="w-4 h-4" /> : <ArrowLeft className="w-4 h-4" />}{t.backClasses}</button>
                    <div className={`rounded-2xl bg-white shadow-sm p-5 flex items-center gap-4 ${rowR}`}>
                      ...
                      <div className={ta}><div className="text-lg font-extrabold text-stone-800">{classDetail.class.name}</div>{classDetail.class.teacher_name && <div className="text-sm text-stone-400">{t.clTeacher}: {classDetail.class.teacher_name}</div>}</div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      <ChartCard title={t.cdScoreDist} ta={ta}>
                        <ResponsiveContainer><BarChart data={classDetail.score_dist} margin={{ top: 6, right: 10, bottom: 0, left: -18 }}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" vertical={false} /><XAxis dataKey="band" tick={{ fontSize: 10 }} stroke="#78716c" /><YAxis tick={{ fontSize: 10 }} stroke="#78716c" allowDecimals={false} /><Tooltip contentStyle={tip} cursor={{ fill: '#f5f5f4' }} />
                    ...
                    {classDetail.assignments.length > 0 && (
                      <div className="rounded-2xl bg-white shadow-sm p-4"><div className={`text-sm font-semibold text-stone-700 mb-3 ${ta}`}>{t.cdAssignments}</div><div className="space-y-3">{classDetail.assignments.map((a, i) => (<div key={`${a.title}-${i}`} className={`flex items-center gap-3 ${rowR}`}><div className={`flex-1 text-sm font-medium text-stone-700 truncate ${ta}`}>{a.title}</div><div className="w-32 h-2 rounded-full bg-stone-100 overflow-hidden shrink-0"><div className="h-full rounded-full bg-gradient-to-r from-emerald-400 to-teal-500" style={{ width: `${a.completion}%` }} /></div><div className="w-10 text-sm font-semibold text-stone-600 shrink-0" style={{ textAlign: isRTL ? 'left' : 'right' }}>{a.completion}%</div></div>))}</div></div>
                    )}
                  </div>
                )
              )}
                  ...
                  <div className="space-y-5">
                    <ChartCard title={t.enActive30} ta={ta}>
                      <ResponsiveContainer><AreaChart data={engagement.active30} margin={{ top: 6, right: 10, bottom: 0, left: -18 }}>
                        ...
                        <CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" vertical={false} /><XAxis dataKey="d" tick={{ fontSize: 9 }} stroke="#78716c" interval={2} /><YAxis tick={{ fontSize: 10 }} stroke="#78716c" allowDecimals={false} /><Tooltip contentStyle={tip} />
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      ...
                      <ChartCard title={t.enDow} ta={ta}>
                        <ResponsiveContainer><BarChart data={dowData} margin={{ top: 6, right: 10, bottom: 0, left: -18 }}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" vertical={false} /><XAxis dataKey="day" tick={{ fontSize: 11 }} stroke="#78716c" /><YAxis tick={{ fontSize: 10 }} stroke="#78716c" allowDecimals={false} /><Tooltip contentStyle={tip} cursor={{ fill: '#f5f5f4' }} />
                    ...
                    <ChartCard title={t.enMode} ta={ta}>
                      <ResponsiveContainer><BarChart data={engagement.modes} layout="vertical" margin={{ top: 0, right: 12, bottom: 0, left: 16 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" horizontal={false} /><XAxis type="number" tick={{ fontSize: 10 }} stroke="#78716c" allowDecimals={false} /><YAxis type="category" dataKey="mode" tick={{ fontSize: 11 }} stroke="#78716c" width={80} /><Tooltip contentStyle={tip} cursor={{ fill: '#f5f5f4' }} />
