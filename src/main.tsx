...
async function manageServiceWorker() {
  ...
  navigator.serviceWorker.addEventListener('controllerchange', () => {
    ...
    // If the tab is hidden (background, phone locked, app switcher) the
    // user isn't mid-game and reloading immediately is safe.  When the tab
    // is visible, defer until the next visibility change to hidden so a
    // deploy can't yank the page out from under a student answering
    // question 8 of 10 or a teacher running a Live Challenge podium.  The
    // fresh SW takes effect the next time they bring the tab to the
    // foreground, with zero interruption to the current session.
    if (document.hidden) {
      performReload();
      return;
    }
