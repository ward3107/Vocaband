...
/**
 * Dashboard "Word Hunt Arena" tap. Like Speed Round there's no word picker —
 * it creates a wordless arena session immediately and drops the teacher into
 * the live host control room (where they pick the set + mode mix + timer and
 * start the arena). Clears any stale QP state first.
 */
export async function startWordHuntArenaFromDashboard(deps: CategoryRaceClickDeps): Promise<void> {
  try { sessionStorage.setItem('vocaband_skip_restore', 'true'); } catch { /* ignore */ }
  try { localStorage.removeItem('vocaband_quick_play_session'); } catch { /* ignore */ }
  deps.cleanupSessionData();
  deps.setQuickPlayActiveSession(null);
  deps.setQuickPlaySessionCode(null);
  try {
    await createWordHuntArenaSession({
      showToast: deps.showToast,
      failedCreateSessionMsg: (err) => `Couldn't start Word Hunt Arena: ${err}`,
      setSessionCode: (code) => deps.setQuickPlaySessionCode(code),
      setActiveSession: (s) => deps.setQuickPlayActiveSession({
        id: s.id,
        sessionCode: s.sessionCode,
    ...
    deps.setView('word-hunt-arena-host');
  } catch {
    /* createWordHuntArenaSession already surfaced a toast */
  }
}
...
export function loadAssignmentIntoCreateForm(
  ...
  // A duplicate must INSERT a new row; only a real edit carries the source
  // row's identity. handleSaveAssignment branches on `editingAssignment` and
  // UPDATEs by that id, so setting it for a duplicate made "Duplicate"
  // OVERWRITE the original assignment — renaming it "… (copy)" and replacing
  // its word list — instead of creating a copy. The original was destroyed
  // with no new row to show for it.
  deps.setEditingAssignment(asDuplicate ? null : assignment);
  // Classify custom words by their OWN identity — negative OCR ids, or the
  // 'Custom' level the vocabulary library mints — rather than purely by
  // ALL_WORDS membership. The vocabulary chunk loads lazily, so on a cold
  // cache `allWords` is [] and EVERY curriculum word looked "unknown": the
  // wizard filed them all as custom and forced the Custom tier. The
  // membership test is kept, but only once the corpus is actually loaded,
  // so a genuinely stale id still survives as a custom word.
  const vocabReady = allWords.length > 0;
  const unknownWords: Word[] = (assignment.words ?? []).filter(
    (w: Word) =>
      w.id < 0 ||
      w.level === 'Custom' ||
      (vocabReady && !allWords.some((aw) => aw.id === w.id)),
  );
  ...
  // The tier heuristic is only meaningful against a loaded corpus — on a cold
  // cache it used to label everything 'Custom'. Leave the picker on its
  // default instead of mislabelling the teacher's set.
  if (vocabReady) {
